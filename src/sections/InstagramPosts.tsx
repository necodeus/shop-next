...
interface IInstagramPostSection {
    posts: any[];
}

export default function InstagramPostsSection<IInstagramPostSection>({ posts = [] }) {
    return (
        <div className="rounded-[17px] overflow-hidden">
            <div className="grid grid-cols-2 md:grid-cols-3">
                {posts.map((post: any, index: any) => <div key={index} className="relative">
                    <Link
                        href={ post.link }
                        target="_blank"
                        rel="noopener"
